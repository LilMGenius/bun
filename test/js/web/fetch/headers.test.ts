...
describe("Headers", () => {
  ...
  describe("append()", () => {
    ...
    // combined value with makeString(), so N appends copied O(N^2) bytes: the
    // ledger measured 200,000 appends at 1.8s and 400,000 at 9.4s on release,
    // where Node takes 0.5s for 400,000. The blocks below cover both halves of
    // the fix: the combined value is still exactly the same string, and the
    // cost of producing it is now linear.
