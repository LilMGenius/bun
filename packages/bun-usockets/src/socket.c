...
int us_socket_write_check_error(struct us_socket_t *s, const char *data, int length, int *fatal_write_error) {
    ...
    if (s->ssl) {
        /* A TLS write reaches the wire from inside the write BIO and from the
         * batch/spill flushes, all of which fold a rejected send into "the
         * wire blocked" (see us_socket_raw_write). us_socket_raw_write records
         * the code instead, so report it here through the same channel plain
         * TCP uses. Without it the caller buffers the bytes as backpressure,
         * the peer's reset is never reported, and the connection ends as a
         * clean close. A failure before the handshake has been reported keeps
         * its existing channel: the handshake dispatch carries the reason. */
        int written = us_socket_write(s, data, length);
        if (s->fatal_send_error) {
            if (fatal_write_error && us_internal_ssl_handshake_callback_has_fired(s)) {
                *fatal_write_error = s->fatal_send_error;
            }
            s->fatal_send_error = 0;
        }
        return written;
    ...
    int written = bsd_send(us_poll_fd(&s->p), data, length);
    if (written < 0) {
        int fatal_errno = us_internal_classify_failed_send(s);
        if (!fatal_errno) {
            s->flags.last_write_failed = 1;
            us_internal_rearm_writable(s);
            return 0;
        ...
        if (fatal_write_error) *fatal_write_error = fatal_errno;
        return 0;
    }
    s->unclassified_send_failures = 0;
    if (written != length) {
...
int us_socket_raw_write(struct us_socket_t *s, const char *data, int length) {
    ...
    int written = bsd_send(us_poll_fd(&s->p), data, length);
    if (written < 0) {
        /* This is the TLS layer's wire: the ciphertext is written from inside
         * the write BIO and from the batch/spill flushes, and a 0 return means
         * "the wire blocked" to all of them. Record a send the kernel rejected
         * outright so us_socket_write_check_error can report it to the caller.
         * That report is the only one there is: the bytes are dropped, and the
         * peer's reset is not seen again before the connection goes away. */
        int fatal_errno = us_internal_classify_failed_send(s);
        if (fatal_errno) s->fatal_send_error = fatal_errno;
    } else {
        s->unclassified_send_failures = 0;
        s->fatal_send_error = 0;
    }
